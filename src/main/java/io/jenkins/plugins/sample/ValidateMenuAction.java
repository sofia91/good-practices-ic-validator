...
public class ValidateMenuAction implements Action {
    ...
    public String getMttr(){
        ...
        return result != null? Util.getTimeSpanString(result) + " días." : "Sin builds erroneos.";
    }
    ...
    public List<String> getViewSpeedBuildStatus() {
        ...
        if ( getLastNBuilds() != 0 ) {
            ...
            description = "Estabilidad: " + (long)cont + " de los " + getLastNBuilds() + " builds no cumplen la practica 7:\"Obtener un build rápido - M.F\"";
        }else{
            array.add("");
        }
        ...
        String titlePractise="Práctica 7: Obtener build rápido - M.F";
        ...
        array.add(String.valueOf(getMaxTimeToBuild())); //tiempo maximo de reaparación
        return array;
    ...
    public List<String> getViewSpeedRepairBuildStatus() {
        ...
        String tabla = "<div data-placement=\"right\"><table><thead><tr style=\"background: #fdffce;border: 2px solid #fcffc4;padding:2%;\"><th scope=\"col\">%</th><th scope=\"col\">Description</th></tr></thead><tbody><tr style=\"background: #ffffff;\"><td>"+result +"</td><td>"+description+"</td></tr></tbody></table></div>";
        String titlePractise="Práctica 6: Reparar builds fallidos immediatamente - M.F";
        array.add(getGenerateWeather(result));//imagen
        array.add(tabla);//tabla
        array.add(String.valueOf(result)); //score
        array.add(titlePractise); //titulo de la practica
        array.add(String.valueOf((getNRepairBuilds() - cont))); //numero de builds que no superan el maximo
        array.add(String.valueOf(getTimeRepairBuilds())); //tiempo maximo de reaparación
        return array;
    }
    ...
    public List<String> getRecepcionDatos(String practica){
        List<String> lista = new ArrayList<String>();
        String estilo = "\"display:block;\"";


        if( practica == "P0"){
            lista.add(estilo);
            lista.addAll(getStatusWeatherJenkins());

        }
        else if( practica == "P6"){
            lista.add(estilo);
            lista.addAll(getViewSpeedRepairBuildStatus());

        }
        else if( practica == "P7"){
            lista.add(estilo);
            lista.addAll(getViewSpeedBuildStatus());
          //  System.out.println("desaparece "+ lista.get(1));
        }
        else {
            estilo = "display:none;";
            lista.add(estilo);
            System.out.println("desaparece "+ lista.get(0));
        }
